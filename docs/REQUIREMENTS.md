...
- 至少包含 3 个商品及对应库存和订单样例。
- 检查标题与五点描述完整度、FBA 低库存和超过 3 天未发货订单。
...
- 每次成功执行保存截图；不得访问真实 Seller Central 或发布商品。
...
3. 正常 Demo 生成 4 个成功步骤、截图和两种报告。
4. 失败 Demo 首次为 `failed`，重试后为 `succeeded`，已完成步骤只执行一次。
5. 无 SP-API 凭证时状态端点可访问，预览端点返回 409 且不触网。
6. 契约测试能把官方字段形态映射为低库存与订单关注项，不包含 PII。
7. `PROJECT_STATUS.md` 分开记录已验证与未验证范围。
